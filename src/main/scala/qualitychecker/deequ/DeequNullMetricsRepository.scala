...
class DeequNullMetricsRepository extends MetricsRepository {
  ...
  override def load(): MetricsRepositoryMultipleResultsLoader = new MetricsRepositoryMultipleResultsLoader {
    override def withTagValues(tagValues: Map[String, String]): MetricsRepositoryMultipleResultsLoader = this

    override def forAnalyzers(analyzers: Seq[Analyzer[_, Metric[_]]]): MetricsRepositoryMultipleResultsLoader = this
