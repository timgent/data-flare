...
case class GenericCheckSuite(
  ...
  override def run(timestamp: Instant)(implicit ec: ExecutionContext): Future[ChecksSuiteResult] = {
    ...
    for {
      ...
      val checkSuiteResult = ChecksSuiteResult(
        ...
        resultDescription = ChecksSuite.getOverallCheckResultDescription(allCheckResults),
   ...
   * @return
   */
  override def qcType: QcType = QcType.GenericChecks
}
