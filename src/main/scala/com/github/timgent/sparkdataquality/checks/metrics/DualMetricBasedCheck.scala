...
final case class DualMetricBasedCheck[MV <: MetricValue](dsAMetricDescriptor: MetricDescriptor,
                                                         ...
                                                         metricComparator: MetricComparator[MV]) extends MetricsBasedCheck {

  override def description: String = s"$userDescription. Comparing metric ${dsAMetricDescriptor.toSimpleMetricDescriptor} to ${dsBMetricDescriptor} using comparator of ${metricComparator.description}"
  ...
  private def getCheckResult(checkPassed: Boolean, dsAMetric: MV, dsBMetric: MV): CheckResult = {
    if (checkPassed)
      CheckResult(qcType, CheckStatus.Success, s"metric comparison passed. dsAMetric of $dsAMetric was compared to dsBMetric of $dsBMetric", description)
    else
      CheckResult(qcType, CheckStatus.Error, s"metric comparison failed. dsAMetric of $dsAMetric was compared to dsBMetric of $dsBMetric", description)
  }
  ...
  def applyCheckOnMetrics(dsAMetrics: Map[MetricDescriptor, MetricValue],
    ...
    (dsAMetricOpt, dsBMetricOpt) match {
      case (Some(dsAMetric), Some(dsBMetric)) => (dsAMetric, dsBMetric) match {
        case (dsAMetric: MV, dsBMetric: MV) => getCheckResult(metricComparator.fn(dsAMetric, dsBMetric), dsAMetric, dsBMetric)
