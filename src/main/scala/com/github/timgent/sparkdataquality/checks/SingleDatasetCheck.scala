...
object SingleDatasetCheck {
  ...
  def sumOfValuesCheck[T: Encoder](
      columnName: String,
      threshold: AbsoluteThreshold[T]
  ): SingleDatasetCheck = {
    val checkDescription =
      s"Constraining sum of column $columnName to be within threshold $threshold"
    SingleDatasetCheck(checkDescription) { ds =>
      ...
      if (withinThreshold)
        RawCheckResult(
          CheckStatus.Success,
          s"Sum of column $columnName was $sumOfCol, which was within the threshold $threshold"
        )
      else
        RawCheckResult(
          CheckStatus.Error,
          s"Sum of column $columnName was $sumOfCol, which was outside the threshold $threshold"
        )
    }
  }
}
