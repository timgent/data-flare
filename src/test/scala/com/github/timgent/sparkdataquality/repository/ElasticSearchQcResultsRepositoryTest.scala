...
class ElasticSearchQcResultsRepositoryTest extends AsyncWordSpec with Matchers with DockerTests with Eventually with EsTestUtils {
  "ElasticSearchQcResultsRepository.save" should {
    ...
    "Append check suite results to the index" in {
      ...
      val checkResultA1 = generateRawCheckResult(SingleDatasetQualityCheck, "A1", CheckStatus.Success)
      val checkResultA2 = generateRawCheckResult(SingleDatasetQualityCheck, "A2", CheckStatus.Success)
      val checkResultB1 = generateRawCheckResult(DatasetComparisonQualityCheck, "B1", CheckStatus.Error)
      val checkResultB2 = generateRawCheckResult(DatasetComparisonQualityCheck, "B2", CheckStatus.Error)
      val checkResultB1Success = generateRawCheckResult(DatasetComparisonQualityCheck, "B1", CheckStatus.Error)
      val checkResultB2Success = generateRawCheckResult(DatasetComparisonQualityCheck, "B2", CheckStatus.Error)
      val initialResultsToInsert: List[ChecksSuiteResult] = List(
        ChecksSuiteResult(Success, "checkSuiteA", "resultA", Seq(checkResultA1, checkResultA2), now, someTags),
        ChecksSuiteResult(Error, "checkSuiteB", "resultB", Seq(checkResultB1, checkResultB2), now, someTags)
      )
