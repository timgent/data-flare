  ...
  "ElasticSearchQcResultsRepository.save" should {
    ...
    "Append check suite results to the index" in {
      ...
      val initialResultsToInsert: Map[DatasetDescription, Map[SimpleMetricDescriptor, MetricValue]] = Map(
        DatasetDescription("dsA") -> Map(SizeMetricDescriptor().toSimpleMetricDescriptor -> LongMetric(1)),
        DatasetDescription("dsB") -> Map(SizeMetricDescriptor().toSimpleMetricDescriptor -> LongMetric(1))
      )
      val moreResultsToInsert: Map[DatasetDescription, Map[SimpleMetricDescriptor, MetricValue]] = Map(
        DatasetDescription("dsA") -> Map(SizeMetricDescriptor().toSimpleMetricDescriptor -> LongMetric(2)),
        DatasetDescription("dsC") -> Map(SizeMetricDescriptor().toSimpleMetricDescriptor -> LongMetric(1))
      )

      def storedResultsFut(): Future[List[(Instant, Map[DatasetDescription, Map[SimpleMetricDescriptor, MetricValue]])]] =
      ...
      for {
        ...
        finalAssertion <- checkStoredResultsAre(storedResultsFut, Map(now -> initialResultsToInsert, later -> moreResultsToInsert).toList)
      } yield {
        finalAssertion
      }
    }
